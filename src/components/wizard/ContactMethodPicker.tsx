...
export function ContactMethodPicker({
  ...
  const availableMethods = allowTypes.filter(
    (type) => !methods.some((existing) => existing.type === type)
  );
  ...
  return (
    <div className="flex-col" style={{ gap: 10 }}>
      ...
      {!disabled && availableMethods.length > 0 && (
        <details ref={menuRef} className="contact-method-add" style={{ marginTop: 4 }}>
          ...
          >
            {availableMethods.map((type) => (
              <button
                ...
                title={t(labelKeyFor(type))}
                onClick={() => addMethod(type)}
                aria-label={t(labelKeyFor(type))}
                style={{ padding: 8, minWidth: 40 }}
              >
                <ContactMethodIcon type={type} size={20} />
              </button>
            ))}
          </div>
        </details>
