 ...
 *
 * Maps each server rejection `code` (the six-code cascade defined in
 ...
 * (`identity` / `describe`) — NOT the friendly review-group names.
 ...
 *
 * Security (T-31-01): the map is a CLOSED `Record` keyed only by the six known
...
/**
 * The six server rejection codes (the `screens.ts` / `persist.ts` cascade
 * order: GROUP_TARGET → INCITEMENT → HATE_TONE → INNOCENT_PARTY →
 * PRIVATE_TARGETING → MISMATCH). Codes outside this set
 * (e.g. VALIDATION_ERROR / INTERNAL_ERROR / NO_SOURCE / WEAK_SOURCE) are
 * deliberately NOT members and resolve to `null` (generic error, stay on review).
 */
...
/**
 * LOCKED UI-SPEC §3 routing table (closed Record keyed by all six codes):
...
/** Type guard: is `code` one of the six known rejection codes? */
